...
- Production bot выпускает challenge только через подписанный внутренний `POST /auth/telegram/challenge` по команде `/start` или `/cabinet` и отправляет Telegram Direct Mini App link с `startapp=<launchId>`. Base URL `https://t.me/<bot_username>/<short_name>` является проверяемой deployment-конфигурацией без query/hash; bot добавляет только одноразовый `launchId`. Публичный self-service challenge запрещён.
