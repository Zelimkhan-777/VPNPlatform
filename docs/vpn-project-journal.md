...
- Выпуск устройства после user advisory lock получает активную Subscription и
  применяемый Plan одним `SELECT … FOR UPDATE`; проверка срока выполняется по
  `CURRENT_TIMESTAMP` PostgreSQL уже внутри транзакции.
- Будущие операции отмены/продления подписки и изменения применяемого тарифа
  должны брать эти же строки в согласованном порядке, поэтому они не смогут
  вклиниться между проверкой подписки и созданием устройства.
- В этом пакете не изменялись публичный API, миграции, реальные ноды,
  VLESS/Xray, платежи или worker processors.
