...
Мобильная совместимость Android/iOS и устойчивость VPN к согласованным сценариям
сетевой фильтрации/блокировок признаны обязательными критериями закрытой беты и
production-readiness. Формулировка о том, что HTTPS/iOS является отдельным
направлением и не влияет на текущий этап, устранена. MVP обязан подтверждать
импорт HTTPS subscription URL и реальный VPN-трафик в актуальном Happ на Android
и iOS, а data-plane profile не считается готовым, если только меняет публичный
IP или работает лишь в свободной сети.

В product, infrastructure и application specifications добавлены требования к
blocked-network/filtering tests из согласованного набора целевых пользовательских
сетей, фиксации handshake/throughput/latency/disconnect/reconnect результатов и
явного release gate перед закрытой бетой. Абсолютное обещание обхода любых
будущих блокировок не вводилось: критерий привязан к проверяемым сетям и
сценариям на момент rollout. Название Astra в owner-ТЗ не добавлялось, так как
это инструмент финального ревью, а не runtime-требование.
