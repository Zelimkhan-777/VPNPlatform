...
- Внутренний сервис поддерживает ротацию, отзыв и проверку только healthy-ноды; публичный endpoint, node-agent и реальные VPN-ноды этим этапом не добавлялись.
