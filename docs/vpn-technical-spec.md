...
- `mymeteora.ru` — выбранный кандидат, но не считается принадлежащим проекту до проверки и покупки оператором. Домены остаются конфигурацией deployment и не хардкодятся в application logic.
...
- [ ] Право на `mymeteora.ru` подтверждено покупкой; root/app/api/sub направлены на production control plane, а `status` включается только при готовности.
