...
Все versioned Compose manifests проходят отдельный offline `docker compose config` guard в локальной проверке и CI с фиксированным non-secret env fixture. Production Xray container имеет Compose healthcheck через loopback-only Handler API: probe проверяет доступность inbound и не публикует служебный port наружу. Этот healthcheck является наблюдаемым container signal и не заменяет node-agent serving verification перед acknowledgement.
