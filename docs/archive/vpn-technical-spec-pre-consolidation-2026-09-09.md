...
Целевая pool role также не кодируется lifecycle-статусом. Членство ноды в логической локации имеет отдельную роль `SERVING` или `STANDBY`: `SERVING` допускает кандидатную выдачу при прохождении остальных gates, `STANDBY` означает тёплый проверяемый резерв без обычной пользовательской выдачи. Вычисляемое здоровье ноды (`UNKNOWN/HEALTHY/DEGRADED/DOWN`) не редактируется OWNER вручную и не подменяет route availability. Persistence foundation содержит `LocationPool` и отдельный `LocationPoolMembership`; для совместимости с прежним единственным `Node.locationLabel` одна нода в MVP имеет не более одного текущего membership. Upgrade группирует существующие ноды по точному location label, связывает каждый pool с последними активированными health/capacity policy versions, сохраняет прежние эксплуатационные ноды как `SERVING`, а незавершённые `PROVISIONING` — как `STANDBY`. Само наличие schema foundation ещё не меняет выдачу: обязательный pool/readiness/capacity gate вводится следующим application-срезом вместе с обновлением всех provisioning/bootstrap paths и тестов.
...
Persistence foundation связывает `Incident` и `NodeOperation` с immutable `AvailabilityDecision`: scope и policy version выводятся только из решения, один decision создаёт не более одного incident и одной operation каждого типа. Automatic materializer сериализуется advisory lock и создаёт `PENDING PROMOTE_STANDBY` intent только когда решение одновременно требует incident/replacement и содержит проверенную policy version. Retry budget берётся из валидированной orchestration-конфигурации. DB constraints/triggers запрещают изменение ownership, scope, policy, idempotency и terminal result, уменьшение attempts, обход переходов `PENDING → RUNNING → SUCCEEDED/FAILED`, удаление timeline и разрешение incident до terminal status всех связанных operations. Resolution timeline event атомарно переводит incident из `OPEN` в `RESOLVED`. Наличие pending intent не запускает ноду и не считается promotion: фактический executor появляется только вместе с `LocationPool`, `SERVING/STANDBY`, readiness/capacity recheck, bounded assignment и convergence.
