...
describe('NodeLifecycleManager', () => {
  ...
  it('drains HEALTHY with one status update and audit', async () => {
    const transaction = {
      $queryRaw: vi
        .fn()
        .mockResolvedValue([{ id: 'node-1', status: 'HEALTHY' }]),
    ...
    await expect(manager.drain('node-1', 'user-1')).resolves.toEqual({
      nodeId: 'node-1',
      status: 'DRAINING',
    });
    expect(transaction.node.update).toHaveBeenCalledWith({
    ...
    expect(transaction.auditEvent.create).toHaveBeenCalledWith({
      data: {
        ...
        entityType: 'Node',
        entityId: 'node-1',
        metadata: { previousStatus: 'HEALTHY' },
      },
    });
  });

  it('keeps repeated drain idempotent without another write', async () => {
    const transaction = {
      $queryRaw: vi
        .fn()
        .mockResolvedValue([{ id: 'node-1', status: 'DRAINING' }]),
      node: { update: vi.fn() },
      auditEvent: { create: vi.fn() },
    };
    const manager = new NodeLifecycleManager(
    ...
    await expect(manager.drain('node-1')).resolves.toEqual({
      nodeId: 'node-1',
      status: 'DRAINING',
    });
    expect(transaction.node.update).not.toHaveBeenCalled();
  ...
  it.each(['DISABLED', 'QUARANTINED', 'DELETED', 'PROVISIONING'])(
    'rejects drain from %s',
    async (status) => {
      const transaction = {
        $queryRaw: vi.fn().mockResolvedValue([{ id: 'node-1', status }]),
        node: { update: vi.fn() },
      ...
      await expect(manager.drain('node-1')).rejects.toThrow(
        'Node cannot be drained',
      );
      expect(transaction.node.update).not.toHaveBeenCalled();
      expect(transaction.auditEvent.create).not.toHaveBeenCalled();
    },
