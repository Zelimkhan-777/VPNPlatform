  ...
  async claim(
    ...
    await this.prisma.$executeRaw`
      ...
      WHERE id = CAST(${command.nodeSyncJobId} AS uuid)
        AND "nodeAccessGrantId" = CAST(${command.nodeAccessGrantId} AS uuid)
        AND "targetVersion" = ${command.targetVersion}
        AND status = 'PROCESSING'
        AND "leaseExpiresAt" <= clock_timestamp()
    `;
    const rows = await this.prisma.$queryRaw<ClaimedNodeSyncJob[]>`
      WITH candidate AS (
        SELECT job.id
        ...
        WHERE job.id = CAST(${command.nodeSyncJobId} AS uuid)
          AND job."nodeAccessGrantId" = CAST(${command.nodeAccessGrantId} AS uuid)
          ...
          AND (job."nextAttemptAt" IS NULL OR job."nextAttemptAt" <= clock_timestamp())
        FOR UPDATE OF job
      )
    ...
    if (
      existing &&
      (existing.nodeAccessGrantId !== command.nodeAccessGrantId ||
        existing.targetVersion !== command.targetVersion)
    ) {
      return 'terminal';
    }
